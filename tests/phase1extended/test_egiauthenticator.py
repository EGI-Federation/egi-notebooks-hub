...
# Purpose: Verify that a newly created authenticator uses the built-in default Check-in
# host.
# Pass example: no EGICHECKIN_HOST environment variable is set, so the host stays
# "aai.egi.eu".
# Fail example: a refactor accidentally changes the default host string or
# leaves it unset.
...
# Purpose: Check that the authenticator honors EGICHECKIN_HOST when no explicit config
# value is given.
# Pass example: EGICHECKIN_HOST=checkin.dev.example results in checkin_host ==
# "checkin.dev.example".
# Fail example: the environment variable is ignored and the code keeps
# using the hard-coded default host.
...
# Pass example: config says "from-config.example" while env says "from-env.example", and
# config wins.
# Fail example: the environment variable silently overrides
# administrator-supplied config.
...
# Purpose: Ensure all important URLs are derived consistently from the chosen Check-in
# host.
# Pass example: checkin.example.org produces matching
# authorize/token/userinfo/introspect/revoke URLs.
# Fail example: one endpoint still points to the old default host or uses
# a wrong path suffix.
...
# Pass example: ["profile", "offline_access"] becomes ["openid", "profile",
# "offline_access"].
# Fail example: the authenticator asks the provider for scopes without
# "openid" and breaks OIDC assumptions.
...
# phase1-6
# Component: EGICheckinAuthenticator scope validation.
# Purpose: Confirm that an already-correct scope is preserved and not duplicated or
# reordered badly.
# Pass example: ["openid", "profile", "offline_access"] stays exactly the same.
# Fail example: validation appends a second "openid" or mutates a valid
# scope unexpectedly.
...
# Fail example: empty scope remains empty, leading to invalid or
# incomplete authentication requests.
...
# Fail example: a default change disables group management and downstream
# group-based logic stops working.
...
# Purpose: Verify that the human-facing username claim has priority over service-style
# fallback claims.
# Pass example: user info contains preferred_username="alice", so username_claim returns
# "alice".
# Fail example: the code ignores preferred_username and falls back to a
# less appropriate identifier.
...
# Purpose: Ensure that service or machine identities can still get a username when
...
# Purpose: Confirm that administrators can rename the claims used for normal users and
# services.
...
# Purpose: Verify that anonymous identities are deterministic and therefore stable
# across repeated logins.
# Pass example: the same anonymous source data always produces the same prefixed
# SHA-256-based username.
# Fail example: two equivalent user_info payloads generate different
# usernames and break account continuity.
...
# Purpose: Ensure that deployments can change the visible prefix used for anonymous
# accounts.
# Pass example: configured prefix "guest" produces usernames starting with "guest-".
# Fail example: the code hard-codes the default prefix and ignores
...
# Purpose: Check that the authenticator refuses to invent anonymous usernames when that
# feature is disabled.
# Pass example: allow_anonymous=False and missing usable claims lead to a None result.
# Fail example: anonymous usernames are still generated despite explicit
# policy disabling them.
...
# Purpose: Verify that no primary group is chosen when user groups do not intersect with
# allowed_groups.
...
# Purpose: Ensure that missing group information is handled safely and simply returns
# None.
...
# phase1-17
# Component: primary group selection.
# Purpose: Verify that primary group priority follows the group order returned by the
# IdP.
# Pass example: with user groups ["vo-1", "vo-2"] and both groups allowed, "vo-1" is
# selected.
# Fail example: the authenticator incorrectly uses allowed_groups ordering
# and selects "vo-2".
...
# Purpose: Verify that the computed primary group is stored in auth_state for later
# consumers.
# Pass example: a user in vo-2 gets auth_state["primary_group"] == "vo-2".
# Fail example: downstream code cannot see the selected primary group
# because it was never saved.
...
# Purpose: Ensure that primary_group is only written when a meaningful matching group
# exists.
# Pass example: a user with only unrelated groups keeps auth_state unchanged.
# Fail example: the code adds a bogus primary_group key even when no
...
# Purpose: Verify that JWT-based access tokens are marked for introspection rather than
# normal OAuth exchange.
...
# Purpose: Confirm that the EGI override only changes the custom path and otherwise
# delegates to the parent class.
# Pass example: when data=None, the parent implementation is called exactly once.
# Fail example: the override breaks standard OAuth behavior by bypassing
# the parent logic.
...
# Purpose: Check that when JWT mode is active, the provided token data is already the
# token info source of truth.
# Pass example: params["data"] is returned directly without external requests.
# Fail example: the code makes unnecessary network calls for information
# already present in the JWT flow.
...
# Purpose: Verify that JWT login uses introspection and passes the token_info payload
# through to it.
# Pass example: token_info with introspect=True results in a call to introspect_token.
# Fail example: JWT tokens are treated like standard OAuth tokens and skip
# the custom introspection path.
...
# Pass example: a normal oauth-token causes GenericOAuthenticator.token_to_user to be
# awaited.
# Fail example: the override hijacks all token flows, even the ones it
...
# Fail example: the code sends an empty introspection request and hides a
# programming error.
...
# Pass example: the final handler list contains /oauth_callback, /jwt_login, and
# /token_revoke.
# Fail example: custom routes are missing or the parent routes are
...
# Purpose: Verify that EOSC-specific group names are reduced to the personal project
# identifier.
...
# Purpose: Verify that custom regexes still work even when they do not define an
# explicit capturing group.
# Pass example: regex ^special-group$ returns "special-group" for that exact group.
# Fail example: custom patterns only work when they contain a capture
# group, reducing configurability.
...
# Purpose: Ensure the EOSC-specific helper does not invent a project when none matches
# the regex.
...
# Purpose: Verify the exact HTTP request sent to the introspection endpoint, including
# Basic Auth.
# Pass example: body is b"token=jwt-token" and Authorization contains
...
# phase1-38
# Component: EGICheckinAuthenticator OpenID Connect discovery.
# Purpose: Ensure all URLs are obtained from the OIDC discovery
# Pass example: checkin.example.org produces matching
# authorize/token/userinfo/introspect/revoke URLs.
# Fail example: one endpoint still points to the old default host or uses
# a wrong path suffix.
def test_urls_are_discovered_from_issuer(auth_config):
    auth_config.EGICheckinAuthenticator.checkin_host = "checkin.example.org"
    auth_config.EGICheckinAuthenticator.use_oidc_discovery = True
    authenticator = EGICheckinAuthenticator(config=auth_config)
    with patch.object(
        HTTPClient,
        "fetch",
    ) as m_fetch:
        base = "https://example.org/oidc"
        m_fetch.return_value = Mock()
        m_fetch.return_value.body.decode.return_value = json.dumps(
            {
                "authorization_endpoint": f"{base}/authz",
                "token_endpoint": f"{base}/token",
                "userinfo_endpoint": f"{base}/info",
                "introspection_endpoint": f"{base}/introspection",
                "revocation_endpoint": f"{base}/revocation",
            }
        )
        assert authenticator.authorize_url == f"{base}/authz"
        assert authenticator.token_url == f"{base}/token"
